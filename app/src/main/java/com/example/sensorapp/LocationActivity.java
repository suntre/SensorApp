...
public class LocationActivity extends AppCompatActivity {
    private Button getLocalisation;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        getLocalisation = findViewById(R.id.localisation_get);
        getAddress = findViewById(R.id.get_address);
        locationTextView = findViewById(R.id.location_text_view);
        addressTextView = findViewById(R.id.address_text_view);
        fusedLocationProviderClient = LocationServices.getFusedLocationProviderClient(this);
        getLocalisation.setOnClickListener(v -> getLocation());
        getAddress.setOnClickListener(v -> executeGeocoding());
    }
    ...
    private String locationGeocoding(Context context, Location location){
        ...
        try{
            ...
            Log.e(SENSOR_LOG_TAG, resultMessage, ioException);
        }

        if(addresses == null || addresses.isEmpty()){
            if(resultMessage.isEmpty()){
                resultMessage = context.getString(R.string.no_address_found);
                Log.e(SENSOR_LOG_TAG, resultMessage);
            }
        }
        else{
            Address address = addresses.get(0);
            List<String> addressesParts = new ArrayList<>();
            for(int i = 0; i <= address.getMaxAddressLineIndex(); i++){
                addressesParts.add(address.getAddressLine(i));
            }
    ...
    private void executeGeocoding(){
        if(lastLocation != null){
            ...
            try{
                ...
                Log.e(SENSOR_LOG_TAG, e.getMessage(), e);
